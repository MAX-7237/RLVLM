...
class PrismaticVLM(VLM):
    ...
    def freeze_backbones(self, stage: str) -> None:
        ...
        if stage == "align":
            ...
            self.vision_backbone.requires_grad_(False)
            if not self.llm_backbone.use_lora:
                self.llm_backbone.requires_grad_(True)
            else:
                pass # will only require grad for trainable params
            self.projector.requires_grad_(True)
    ...
    def forward(
        ...
        with torch.set_grad_enabled(self.vision_backbone_requires_grad):
            if isinstance(pixel_values, dict):
                patch_features_before_clustering = self.vision_backbone({k: pixel_values[k][multimodal_indices] for k in pixel_values})
            else:
                patch_features_before_clustering = self.vision_backbone(pixel_values[multimodal_indices])

        patch_features = patch_features_before_clustering
        ...
        # Align dtypes and sanitize to avoid NaNs propagating downstream
        if projected_patch_embeddings.dtype != input_embeddings.dtype:
            projected_patch_embeddings = projected_patch_embeddings.to(dtype=input_embeddings.dtype)
        # Replace NaN/Inf with finite numbers to stabilize early steps
        projected_patch_embeddings = torch.nan_to_num(projected_patch_embeddings, nan=0.0, posinf=1e6, neginf=-1e6)
        input_embeddings = torch.nan_to_num(input_embeddings, nan=0.0, posinf=1e6, neginf=-1e6)

        # Build Multimodal Embeddings (and build resulting attention mask)
        device=input_embeddings.device
        multimodal_indices = multimodal_indices.to(device)
        projected_patch_embeddings = projected_patch_embeddings.to(device)
        ...
        # Build image token mask for LLM pruning
        batch_size, seq_len, _ = fused_embeddings.shape
        image_token_mask = torch.zeros(batch_size, seq_len, dtype=torch.bool, device=fused_embeddings.device)
        image_token_count = projected_patch_embeddings.shape[1] if projected_patch_embeddings is not None else 0

        if len(multimodal_indices) > 0 and image_token_count > 0:
            ...
            multimodal_mask[:, 1 : 1 + image_token_count] = True

            if len(unimodal_indices) > 0:
                unimodal_mask = torch.zeros(
                    unimodal_embeddings.shape[0],
                    unimodal_embeddings.shape[1],
                    dtype=torch.bool,
                    device=fused_embeddings.device,
                )
                image_token_mask = torch.vstack([multimodal_mask, unimodal_mask])
            else:
                image_token_mask = multimodal_mask
        ...
        with torch.autocast("cuda", dtype=autocast_dtype, enabled=self.enable_mixed_precision_training):
            for idx, input_ids in enumerate(batch_input_ids):
                ...
                if return_string_probabilities is None:
                    full_out_ids = super().generate(input_ids=input_ids, pixel_values=pixel_values, pad_token_id=tokenizer.pad_token_id, **kwargs)
