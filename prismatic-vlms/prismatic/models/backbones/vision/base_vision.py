...
from prismatic.models.backbones.vision.rl_pruner import MAPPOPruningConfig, MAPPOPruningController
...
class TimmViTBackbone(VisionBackbone, ABC):
    def __init__(
        ...
        self.rl_pruning_enabled = rl_pruning
        self.pruning_controller: Optional[MAPPOPruningController] = None
        if self.rl_pruning_enabled:
            pruning_config = MAPPOPruningConfig(
                target_layers=rl_target_layers,
                threshold=rl_threshold,
                min_keep_ratio=rl_min_keep_ratio,
                policy_path=rl_policy_path,
            )
            self.pruning_controller = MAPPOPruningController(
    ...
    def _register_pruning_hooks(self) -> None:
        if self.pruning_controller is None:
            return

        total_blocks = len(self.featurizer.blocks)
        for layer_idx in self.pruning_controller.target_layers:
            block_index = layer_idx - 1  # target layers are 1-based
            if block_index < 0 or block_index >= total_blocks:
                warnings.warn(
                    f"Skipping invalid pruning layer `{layer_idx}` for backbone `{self.identifier}` "
                    f"(model has {total_blocks} blocks).",
                    RuntimeWarning,
                )
                continue

            block = self.featurizer.blocks[block_index]
            block.attn.fused_attn = False
            wrapped_forward = self.pruning_controller.build_attention_forward(block.attn, layer_idx)
            block.attn.forward = types.MethodType(wrapped_forward, block.attn)