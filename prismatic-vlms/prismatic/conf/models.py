...
class ModelConfig(ChoiceRegistry):
    ...
    # used for dynamic visual tokens  (dvt, ie Chat-UniVi)
    use_cluster: bool = False                               # Whether to use dynamic patch clustering
    num_scale_reps: int = 3                                 # Number of scale represenations for patch clustering
    cluster_rate0: float = 64                               # Cluster sampling rate for the first scale representation
    cluster_rate1: float = 32                               # Cluster sampling rate for the second scale representation
    cluster_rate2: float = 16                               # Cluster sampling rate for the third scale representation
    cluster_center_features: str = "siglip"                 # Features to use for cluster center computation
    token_assignment_features: str = "siglip"               # Features to use for token assignment computation
    aggregation_features: str = "siglip"                    # Features to use for aggregation computation

    # Reinforcement learning based dynamic token pruning
    vision_rl_pruning: bool = True
    vision_rl_target_layers: tuple[int, ...] = (3, 6, 9)
    vision_rl_threshold: float = 0.5
    vision_rl_min_keep_ratio: float = 0.1
    vision_rl_policy_path: Optional[str] = None

    # LLM-side pruning actors
    llm_pruning_actor_layers: Optional[tuple[int, ...]] = None
    llm_pruning_actor_hidden_dim: Optional[int] = None
    llm_pruning_actor_num_samples: Optional[int] = None
    llm_pruning_reward_alpha: float = 1.0
    llm_pruning_reward_beta: float = 1.0
